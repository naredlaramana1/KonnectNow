...
using KonnectNow.Repository.Repositories;
...
namespace KonnectNow.WebAPI.Managers
{
    ...
    public class MessageManager : BaseModelManager, IMessageManager
    {
        ...
        private readonly IChatStatusRepository _chatStatusRepository;
        ...
        /// <param name="chatStatusRepository">IChatStatusRepository object</param>
                            ...
                            )
        {
            ...
            _chatStatusRepository = chatStatusRepository;
        }
        /// <summary>
        ...
        public ModelManagerResult<SellerRespondMessageViewModel> GetSellerRespondMessages(long queryId, long userId)
        {
            ...
            foreach (var item in distinctMessageList)
            {
                ...
                var user = messages.Where(x => x.User1.UserId == item).First().User1;
                var chatstatus = _chatStatusRepository.Get(x => x.QueryId ==queryId && x.FromUserId == userId && x.ToUserId == item).SingleOrDefault();
                sellerRespondMessageSearchCollection.SearchResults.Add(new SellerRespondMessageInfo { Message = message.Text, MessageCount = messages.Count, UserId = Convert.ToInt64(item), UserName = user.FirstName + " " + user.LastName, IsConnected=chatstatus!=null });
        ...
        public ModelManagerResult<UserRespondMessageViewModel> GetUserRespondMessages(long userId)
        {
            ...
            foreach (var item in distinctMessageList)
            {
                ...
                var message = messages.First();
                var user = messages.Where(x => x.User1.UserId == item).First().User1;
                 var chatstatus = _chatStatusRepository.Get(x => x.QueryId==message.QueryId&& x.FromUserId == Convert.ToDecimal(userId) && x.ToUserId == Convert.ToDecimal(item)).SingleOrDefault();
                 userRespondMessageSearchCollection.SearchResults.Add(new UserRespondMessageInfo { Message = message.Text, MessageCount = messages.Count, UserId = Convert.ToInt64(item), UserName = user.FirstName + " " + user.LastName, QueryId = Convert.ToInt64(message.QueryId), QueryText = message.Query.QueryText, IsConnected = chatstatus!=null });
        ...
        [UnitOfWork]
        public ModelManagerResult<bool> DeleteConversion(long queryId, long fromUserId, long toUserId)
        {
            if (_queryRepository.GetByID(queryId) == null)
        ...
        /// <returns>ModelManagerResult(Boolean)</returns>
        [UnitOfWork]
        public ModelManagerResult<bool> ConnectUser(long queryId, long fromUserId, long toUserId)
        {
            if (_queryRepository.GetByID(queryId) == null)
                return GetManagerResult<bool>(ResponseCodes.QUERY_NOT_EXIST);
            if (_userRepository.GetByID(fromUserId) == null || _userRepository.GetByID(toUserId) == null)
